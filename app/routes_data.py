...
RESOURCES = [
    Resource("identity", "identity_items",
             ["kind", "value", "label", "notes", "is_primary"], ["kind", "value"]),
    Resource("accounts", "accounts",
             ["service", "identity_item_id", "url", "status", "recovery_email",
              "recovery_phone", "has_2fa", "recovery_status", "last_verified", "notes"],
             ["service"]),
    Resource("timeline", "timeline_events",
             ["event_type", "ref_table", "ref_id", "occurred_at", "title", "detail"],
             ["event_type", "title"]),
    Resource("notes", "notes", ["ref_table", "ref_id", "body"], ["body"], has_updated=True),
    Resource("graph/nodes", "graph_nodes",
             ["node_type", "label", "ref_table", "ref_id"], ["node_type", "label"]),
...
def _make_routes(res: Resource) -> None:
    ...
    async def _list(_t: str = security.Auth) -> list[dict]:
        return [dict(r) for r in db.query(f"SELECT * FROM {res.table} ORDER BY id DESC")]
